...
function StatusIcon({ status }: { status: TaskStatus }) {
  ...
  if (status === "in-progress") {
    return <Clock className="h-[18px] w-[18px] shrink-0 text-amber-500" />;
  }
  return <Circle className="h-[18px] w-[18px] shrink-0 text-muted-foreground/40" strokeWidth={1.5} />;
}
  ...
  return (
    <div className="relative" ref={ref}>
      <button
        ...
        className="flex items-center justify-center h-7 w-7 rounded-lg opacity-0 group-hover:opacity-100 transition-all duration-200 hover:bg-accent cursor-pointer"
      ...
      {open && (
        <div
          className="absolute top-full right-0 mt-1 min-w-[160px] z-50 rounded-2xl border border-border backdrop-blur-lg shadow-lg p-1 bg-card/95"
        >
          <button
            onClick={(e) => { e.stopPropagation(); onEdit(); setOpen(false); }}
            className="w-full flex items-center gap-2 px-3 py-2 rounded-xl text-[13px] transition-colors cursor-pointer hover:bg-accent text-foreground"
          >
          ...
          <button
            ...
            className="w-full flex items-center gap-2 px-3 py-2 rounded-xl text-[13px] transition-colors cursor-pointer hover:bg-red-500/10 text-red-500"
          >
...
export function TaskListView({
  ...
  const SortIndicator = useCallback(({ field }: { field: SortField }) => {
    if (sortField !== field) {
      return <ChevronDown className="h-3 w-3 ml-1 opacity-0 group-hover:opacity-50 text-muted-foreground" />;
    }
    return sortDir === "asc" ? (
  ...
  return (
    ...
    >
      {/* Table Header */}
      <div
        className="hidden sm:grid items-center px-5 py-3 bg-muted/30"
        style={{
      ...
      >
        ...
        <button
          ...
          className="group flex items-center text-xs uppercase font-semibold tracking-wider transition-colors text-left cursor-pointer text-foreground"
        >
          Task <SortIndicator field="title" />
        </button>
        <button
          onClick={() => handleSort("assignee")}
          className="group flex items-center text-xs uppercase font-semibold tracking-wider transition-colors cursor-pointer text-foreground"
        >
      ...
      {sorted.length === 0 && (
        <div className="flex flex-col items-center justify-center py-16">
          <div
            className="h-12 w-12 rounded-full flex items-center justify-center backdrop-blur-lg border border-border bg-card/50"
          >
      ...
      {sorted.map((task) => {
        ...
        return (
          <div key={task.id}>
            {/* Desktop row */}
            <div
              className="hidden sm:grid group items-center px-5 py-3.5 transition-colors cursor-pointer hover:bg-accent/50 border-b border-border/50"
              style={{
                gridTemplateColumns: "40px 1fr 140px 120px 90px 90px 40px",
                gap: "8px",
              }}
            ...
            >
              ...
              {/* Task title */}
              <p
                className={cn("text-sm font-medium truncate", task.status === "done" && "text-muted-foreground line-through")}
              >
                {task.title}
              </p>
            ...
            {/* Mobile card */}
            <div
              className="sm:hidden flex items-start gap-3 px-4 py-3.5 cursor-pointer border-b border-border/50"
              onClick={() => onEditTask(task)}
            >
              <button
                onClick={(e) => { e.stopPropagation(); onToggleStatus(task.id); }}
              ...
              <div className="flex-1 min-w-0">
                <p
                  className={cn("text-sm font-medium leading-snug", task.status === "done" && "text-muted-foreground line-through")}
                >
                  {task.title}
                </p>
